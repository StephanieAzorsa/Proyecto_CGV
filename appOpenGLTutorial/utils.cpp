...
void Utils::printShaderLog(GLuint shader) {
    ...
    if (len > 0) {
        log = (char*)malloc(len);
         f->glGetShaderInfoLog(shader, len, &chWrittn, log);
...
GLuint Utils::prepareShader(int shaderTYPE, const char* shaderPath)
{
    ...
    GLuint shaderRef = f->glCreateShader(shaderTYPE);
    f->glShaderSource(shaderRef, 1, &shaderSrc, NULL);
    f->glCompileShader(shaderRef);
    checkOpenGLError();
    f->glGetShaderiv(shaderRef, GL_COMPILE_STATUS, &shaderCompiled);
    if (shaderCompiled != 1)
    {
...
void Utils::printProgramLog(int prog) {
    ...
    int chWrittn = 0;
    char* log;
     f->glGetProgramiv(prog, GL_INFO_LOG_LENGTH, &len);
    if (len > 0) {
        log = (char*)malloc(len);
         f->glGetProgramInfoLog(prog, len, &chWrittn, log);
...
int Utils::finalizeShaderProgram(GLuint sprogram)
{
    ...
    checkOpenGLError();
     f->glGetProgramiv(sprogram, GL_LINK_STATUS, &linked);
    if (linked != 1)
    {
...
GLuint Utils::createShaderProgram(const char* vp, const char* fp) {
    ...
    GLuint vfprogram =  f->glCreateProgram();
     f->glAttachShader(vfprogram, vShader);
     f->glAttachShader(vfprogram, fShader);
...
GLuint Utils::createShaderProgram(const char* vp, const char* gp, const char* fp) {
    ...
    GLuint fShader = prepareShader(GL_FRAGMENT_SHADER, fp);
    GLuint vgfprogram =  f->glCreateProgram();
     f->glAttachShader(vgfprogram, vShader);
     f->glAttachShader(vgfprogram, gShader);
     f->glAttachShader(vgfprogram, fShader);
...
GLuint Utils::createShaderProgram(const char* vp, const char* tCS, const char* tES, const char* fp) {
    ...
    GLuint fShader = prepareShader(GL_FRAGMENT_SHADER, fp);
    GLuint vtfprogram =  f->glCreateProgram();
     f->glAttachShader(vtfprogram, vShader);
     f->glAttachShader(vtfprogram, tcShader);
     f->glAttachShader(vtfprogram, teShader);
     f->glAttachShader(vtfprogram, fShader);
...
GLuint Utils::createShaderProgram(const char* vp, const char* tCS, const char* tES, char* gp, const char* fp) {
    ...
    GLuint gShader = prepareShader(GL_GEOMETRY_SHADER, gp);
    GLuint fShader = prepareShader(GL_FRAGMENT_SHADER, fp);
    GLuint vtgfprogram =  f->glCreateProgram();
     f->glAttachShader(vtgfprogram, vShader);
     f->glAttachShader(vtgfprogram, tcShader);
     f->glAttachShader(vtgfprogram, teShader);
     f->glAttachShader(vtgfprogram, gShader);
     f->glAttachShader(vtgfprogram, fShader);
